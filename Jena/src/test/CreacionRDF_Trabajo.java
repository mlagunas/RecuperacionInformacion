...
public class CreacionRDF_Trabajo {
	...
	public static Model generarEjemplo() {
		...
		Property format = model
				.createProperty("http://purl.org/dc/elements/1.1/format");
		...
		Property broader = model.createProperty(skos + "broader"); // PERTENECE
																	// A
																	// (SUBCLASE
																	// DE)
		Property narrower = model.createProperty(skos + "narrower"); // SUPERCLASE
																		// DE
		...
		model.add(scheme, RDF.type, conceptScheme);
		model.add(scheme, DC.title, "Tipos de trabajos");

		Resource trabajo = model.createResource()
				.addProperty(RDF.type, skosConcept).addProperty(inScheme, scheme)
				.addProperty(prefLabel, "Trabajo");

		Resource TFG = model.createResource().addProperty(RDF.type, skosConcept)
				.addProperty(inScheme, scheme).addProperty(prefLabel, "TFG")
				.addProperty(altLabel, "Trabajo Fin de Grado")
				.addProperty(broader, trabajo);

		Resource TFM = model.createResource().addProperty(RDF.type, skosConcept)
				.addProperty(inScheme, scheme).addProperty(prefLabel, "TFM")
				.addProperty(altLabel, "Trabajo Fin de Master")
				.addProperty(broader, trabajo);

		Resource PFC = model.createResource().addProperty(RDF.type, skosConcept)
				.addProperty(inScheme, scheme).addProperty(prefLabel, "PFC")
				.addProperty(altLabel, "Proyecto Fin de Carrera")
				.addProperty(broader, trabajo);

		Resource t1 = model.createResource(trabajoURI).addProperty(DC.title, t)
				.addProperty(DC.creator, author).addProperty(DC.type, ty);
